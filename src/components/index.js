...
export const HrDotted = styled.div`
    ...
    /* background-color: ${ p => p.color || '#335757' }; */
    ${p => p.padding ? `padding: ${p.padding[0] || 0}rem ${p.padding[1] || 0}rem`:''};
    background-image: linear-gradient(to right, ${ p => p.theme.color[p.color || 0] } 0%, ${ p => p.theme.color[p.color || 0] } 50%, transparent 50%);
    // 设置图片宽度与高度
...
export const WingBlank = styled.div`
...
`
export const FlexBlockAuto = styled(FlexBlock) `
    width:auto;
`
